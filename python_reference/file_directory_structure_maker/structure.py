...
class Structure:
    ...
    def save_to_json_file(self, file):
        pathlib.Path(file).write_text(self.to_json())
    ...
    def _permutations_from_sections(self, sections) -> list[dict]:
        return list(
            itertools.chain.from_iterable(
                [[{k: v for k, v in perm} for perm in itertools.permutations(section.items())] for section in sections]
            )
        )
    ...
    def _build_string_groups(self, combinations: dict) -> dict:
        return {self._separator.join(k): [self._separator.join(t) for t in v] for k, v in combinations.items()}
    ...
    def _create_tree_structure(self, tree_group: dict, prefix: str = ''):
        ...
        for pointer, section in zip(pointers, tree_group, strict=True):
            ...
            if isinstance(tree_group.get(section), dict):  # extend the prefix and recurse:
                ...
                yield from self._create_tree_structure(tree_group[section], prefix=prefix + extension)

    def create_directories(self, group: str, base_path: str = '.'):
        if not (grp := self.string_groups.get(group)):
            raise KeyError(f'No group with structure: {group}')
        paths = [pathlib.Path(base_path, *section.split(self.separator)) for section in grp]
        for path in paths:
            path.mkdir(parents=True, exist_ok=True)

    def create_files(self, group: str, base_path: str = '.'):
        if not (grp := self.string_groups.get(group)):
            raise KeyError(f'No group with structure: {group}')
        for string in grp:
